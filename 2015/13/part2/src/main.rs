...
fn main() {
    ...
    let mut happiness_chart: HashMap<String, HashMap<String, i64>> = 
        HashMap::new();
    // Add myself into the seating chart.
    happiness_chart.insert(String::from("me"), HashMap::new());
    ...
    for line in lines.map_while(Result::ok) {
            ...
            match happiness_chart.get(&parts["person1"])
        {
            Some(p) => p.clone(),
            None => {
                // this person hasn't been seen before, so add them to my 
                // preferences with 0 score
                let mut mypref: HashMap<String, i64> = happiness_chart
                    .get("me").unwrap().clone();
                mypref.insert(String::from(&parts["person1"]), 0);
                happiness_chart.insert(String::from("me"), mypref);

                let mut new_people: HashMap<String, i64> = HashMap::new();
                // Add me to their preferences with 0 score
                new_people.insert(String::from("me"), 0);
